<?php
	...
	if(IS_AJAX){ // check if request is ajax or not
		...
		if(isset($_POST['area_data'])){
			$college = selectfunction('*',COLLEGE,'college_area_id ="'.$_POST['area_id'].'" and college_status ="1"');
			while($row = mysql_fetch_array($college)){
		...
		if(isset($_POST['state_data'])){
			$college = selectfunction('*',AREA,'area_state_id ="'.$_POST['state_id'].'" and area_status ="1"');
			while($row = mysql_fetch_array($college)){
		...
		if(isset($_POST['cost_estimation_per_page'])){
			...
			$amount_data = mysql_fetch_array($amount_per_page);
			if(mysql_num_rows($amount_per_page) == 1){
