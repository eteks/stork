...
{
	if ($variable == "search") $search = $value;
	else if ($variable == "p") $pageNo = $value;
	if ($search != "" && $pageNo == "") $qs = "&search=$search";
	if ($search == "" && $pageNo != "") $qs = "&p=$pageNo";
	if ($search != "" && $pageNo != "") $qs = "&search=$search&p=$pageNo";
}
if (isset($_GET['SortOrder']))
{
	if (trim($_GET['SortOrder']) == "ASC" || trim($_GET['SortOrder']) == "DESC") 
	$_SESSION['SortOrder'] = trim($_GET['SortOrder']);
}
$search_string = $_GET['search'];
if (isset($_GET['SortBy']))
{
	if (trim($_GET['SortBy']) == "id" || trim($_GET['SortBy']) == "clicks") 
	if (trim($_GET['SortBy']) == "id") $_SESSION['SortBy'] = "id";
	else if (trim($_GET['SortBy']) == "clicks") $_SESSION['SortBy'] = "clicks";
...
if(isset($_GET['cat']) && categoryExists($_GET['cat']))
$cid = $_GET['cat'];
$page = 1; 
$limit = 10; 
$next = 2;
...
{
	$data = mysqlQuery("SELECT `id` FROM `products` WHERE `title` like '%" . $_GET['search'] . "%'");
}
else if($cid)
	$data = mysqlQuery("SELECT `id` FROM `products` WHERE `cid`=".$cid);
else
...
if($isDeleted)
{
	$message = "<div class='alert alert-success'><li class='fa fa-check-square-o'></li><b> Product Deleted Successfully</b></div>";
}
if(isset($_GET['areDeleted']) && $_GET['areDeleted']==1)
{
	$message ="<div class='alert alert-success'><li class='fa fa-check-square-o'></li><b> Products Deleted Successfully</b></div>";
}
if($isUpdated)
{
	$message = "<div class='alert alert-success'><li class='fa fa-check-square-o'></li><b> Product Updated Successfully</b></div>";
}
if(isset($_GET['areUpdated']) && $_GET['areUpdated']==1)
{
	$message = "<div class='alert alert-success'><li class='fa fa-check-square-o'></li><b> Products Updated Successfully</b></div>";
}
?>	
...
<body>
  <script type="text/javascript">
    ...
    $( "#deletevideo" ).hide();
    $("button[name='delete_selected']").click(function() {  // triggered submit for delete button
  ...
  <script type="text/javascript">
    $( document ).ready(function() {
    $( "#deletevideo" ).hide();
    $("button[name='update_selected']").click(function() {  // triggered submit for delete button
    	...
    	$('.selectedId').change(function () {
		   var selectedlength=$('.selectedId').filter(":checked").length;
		   if(selectedlength > 0)
			$('.showbuttons').show();
			else
			$('.showbuttons').hide();
		...
		$('#deleteSelected').click(function() {
			...
			{
				var totallength=$('input[type=checkbox]:checked').length;
				var the_id = $(this).val();
				$.ajax({
					type: "POST",
					url: "delete.php",
					...
					success: function(n)
					{
						...
						if(count==totallength) {
							...
							if ((url.indexOf("&areDeleted=1") >= 1) || (url.indexOf("?areDeleted=1") >= 1)){
								window.location=url;
							} else if(len>1){
								window.location=url+'&areDeleted=1';
							} else {
								window.location=url+'?areDeleted=1';
							}
						}
				...
				$.ajax({
					...
					success: function(n)
					{
						...
						if(count==totallength) {
							...
							var check=url.split("?");
							var len=check.length;
							if ((url.indexOf("&areUpdated=1") >= 1) || (url.indexOf("?areUpdated=1") >= 1)){
								window.location=url;
							} else if(len>1){
								window.location=url+'&areUpdated=1';
							} else {
								window.location=url+'?areUpdated=1';
							}
						}
	...
	<input type="hidden" id="qs" value="<?php echo $qs ?>" />
		<div class="page-title">
			...
			<form action="products.php" method="GET" id="products_form">
				<div class="input-group search_input_group">
					...
					{
						?>
						<input class="form-control search_textbox typeahead" id="products_search" placeholder="Search a product..." name="search" type="text" value="<?php
						echo ($_GET['search']);
						?>" required />
						<?php
					}
					...
					{
						?>
						<input class="form-control search_textbox typeahead" id="products_search" placeholder="Search a product..." name="search" type="text" required>
						<?php
					}
					?>								
					<span class="input-group-btn">
						<button class="btn btn-info search_button" type="submit"><i class="fa fa-search"></i></button>
					</span>
				</div>
			</form>
		</div>
		...
		<div class="row">
			<div class="col-md-12 col-lg-12">
				<div class="awidget heightset">
					<div class="awidget-head"></div>
					<?php if($message) echo $message; ?>
					<div style="display:none" class='alert alert-success update-wait'><i class="fa fa-spinner fa-spin"></i><b> Please Wait While Products Being Updated...</b></div>
					<div class="awidget-body no-table-bor">
						<?php
						if (isset($_GET['search'])) 
							echo ('<form action="products.php?search=' . $_GET['search'] . '" method="post">');
						else 
						...
						// 	$sql = "SELECT * FROM `products` ORDER BY " . $SortBy . " " . $_SESSION['SortOrder'] . " limit " . $start_result . "," . $limit;
							...
							$sql = "SELECT * FROM `stork_paper_side`";
						}
						...
						if ($count_rows > 0)
						{
									...
									<tbody>
										...
										{	
											...
											if ($title != "")
											{
												...
												<td><a title="' . $fetch['paper_side'] . '" href="' . rootpath() . "/product/" . $fetch['permalink']. '.html">');
												if(strlen($fetch['paper_side'])>30)
													echo substr($fetch['paper_side'],0,30)."...";
												else
													echo $fetch['paper_side'];
												echo ('</a></td>
												<td><a target="_blank" href="http://'.getdomain($fetch['url']).'">');
												if($fetch['paper_side_status']==1)
													echo "Active";
												else
													echo "InActive";
												echo ('</a></td>
												...
												<a href="edit_paper_side.php?id=' . $fetch['paper_side_id'] . '" class="btn  btn-primary btn-xs" title="Edit ' . $row['title'] . '"><i class="fa fa-pencil-square-o "></i> </a>  
												<a  id="delete" data-toggle="modal" href="#myModal1" data-id="' . $fetch['paper_side_id'] . '" title="Delete" class="btn btn-xs btn-danger delete" title="Delete ' . $row['title'] . '"><i class="fa fa-trash-o"></i> </a></td>');
												echo '</tr>';
											}
											$i+= 1;
											?>
										...
										<div class="modal fade" id="myModal0" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
											<div class="modal-dialog">
												<div class="modal-content">
													...
													<div class="modal-body">
														<input type="hidden" name="update" id="vId1" value=""/>
														<center>
															<h5>Are you sure you want to update this Product? </h5>
														</center>
													</div>
													<div class="modal-footer">
														<a name="action" id="up_link" class="btn btn-success" href="">Yes</a>
														<button type="button" class="btn btn-info" data-dismiss="modal">No</button>
													</div>
												</div><!-- /.modal-content -->
											</div><!-- /.modal-dialog -->
											...
											<div class="modal-dialog">
												<div class="modal-content">
													...
													<div class="modal-footer">
														...
														<a class="btn btn-danger" data-dismiss="modal" id="deleteSelected">Yes</a>
														<button type="button" class="btn btn-info" data-dismiss="modal">No</button>
													</div>
												</div><!-- /.modal-content -->
											</div><!-- /.modal-dialog -->
										...
										<div class="modal fade" id="myModal3" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
											<div class="modal-dialog">
												<div class="modal-content">
													...
													<div class="modal-body">
														<center>
															<h5>Are you sure you want to Update selected products? </h5>
														</center>
													</div>
													<div class="modal-footer">
														<a class="btn btn-success" data-dismiss="modal" id="updateSelected">Yes</a>
														<button type="button" class="btn btn-info" data-dismiss="modal">No</button>
													</div>
												</div><!-- /.modal-content -->
											</div><!-- /.modal-dialog -->
							...
							if ($rows > $limit) 
							{
								...
								<ul class="pagination">
									<?php
									if (isset($_GET['search'])) 
										echo ('<li><a href="./products.php?search=' . $_GET['search'] . '" class="page gradient">First</a></li>');
									else 
										echo ('<li><a href="./products.php" class="page gradient">First</a></li>');
									if ($page > 1)
									{
										$i = ((int)$page / 5) * 5 - 1;
										if (!($i + 5 > $last))
									...
									for ($i; $i <= $temp_last; $i++)
									{
										...
										{
											if ($i == $page) 
												echo ('<li class="active"><a href="./products.php?p=' . $i . '&search=' . $search_string . '" class="page active">' . $i . '</a></li>');
											else 
												echo ('<li><a href="./products.php?p=' . $i . '&search=' . $search_string . '" class="page gradient">' . $i . '</a></li>');
										}
										else
										{
											if ($i == $page) 
												echo ('<li class="active"><a href="./products.php?p=' . $i . '" class="page active">' . $i . '</a></li>');
											else 
												echo ('<li><a href="./products.php?p=' . $i . '" class="page gradient">' . $i . '</a></li>');
										}
									}
									if (isset($_GET['search'])) 
										echo ('<li><a href="./products.php?p=' . $last . '&search=' . $search_string . '" class="page gradient">Last</a></li>');
									else 
										echo ('<li><a href="./products.php?p=' . $last . '" class="page gradient">Last</a></li>');
									?>
								</ul>
								<?php 
								} 
					...
					{
						echo ('<div style="padding-top:25px;padding-bottom:30px;text-align:center"><h3>No Paperside Found</h3></div>');
					}
