...
<title>All Cabin Schedule Time</title>
</head>
<body>
...
{
	$val = $_GET['delete'];
	mysqlQuery("DELETE FROM `stork_cabin_schedule_time` WHERE `schedule_time_id`='$val'");
	$isDeleted = true;
	$deleteProduct = true;
}
?>
...
<div class="container" style="margin-top:20px;">   
	...
	<div class="heading_section col-md-12">
		<h3 class="acc-title lg clone_heading"> Cabin Schedule Time</h3>
		<div class="clear_both"> </div>
	</div>
	<div class="add_section">
		<a href="add_cabin_schedule_time.php"> <span> Add </span><span>[+]</span> </a>
	</div>
							    ...
							    <tr class="">						            
						            <td><span class="nobr"><?php 
							            foreach ($timing_type as $key => $value) {
							            	if($key == $fetch['schedule_time_timing_type'])
							            		echo $value;
							            }
						            ?>
									</span></td>
						            <td><span class="nobr"><?php echo $fetch['schedule_time_start'] ?></span></td>
						            <td><span class="nobr"><?php echo $fetch['schedule_time_end'] ?></span></td>
						            <td>
						            	<span class="price">
							            	<?php if($fetch['schedule_time_status']==1)
													echo "Active";
												  else
													echo "InActive";
											?>
						            ...
						            <td class="table_action th_hidden a-center last">     
						                ...
						                ?>
							                <span class="nobr">
								                	<a title="Edit" class="btn btn-primary btn-xs" href="edit_cabin_schedule_time.php?id=<?php echo $fetch['schedule_time_id'] ?>"><i class="fa fa-pencil-square-o "></i> </a>    
									            <span class="separator"></span> 
									            <span class="restrict">      
									             	<a class="btn btn-xs btn-danger delete" title="Delete" data-id="<?php echo $fetch['schedule_time_id'] ?>"><i class="fa fa-trash-o">
									        ...
									        <span class="nobr">
							                	<a title="Edit" class="btn btn-primary btn-xs" href="edit_cabin_schedule_time.php?id=<?php echo $fetch['schedule_time_id'] ?>"><i class="fa fa-pencil-square-o "></i> </a>
								                <span class="separator"></span> 
								             	<a class="btn btn-xs btn-danger delete" title="Delete" data-id="<?php echo $fetch['schedule_time_id'] ?>" href="#myModal1" data-toggle="modal" id="delete"><i class="fa fa-trash-o"></i> </a>
								            </span>
							            <?php } ?>
							        </td>
	...
	<script type="text/javascript" >
		...
		$("#del_link").prop("href", "cabin_schedule_time.php?delete="+myId);
		});
	</script>
	<!-- Delete popup Start -->
